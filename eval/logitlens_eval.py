...
def setup_model(
    ...
    # Load configuration and potentially modify it
    config = AutoConfig.from_pretrained(
        model_path if not finetuned else base_model_name, trust_remote_code=True
    )
    # Try to prevent nnsight from defaulting to sliding_window cache
    if hasattr(config, 'sliding_window') and config.sliding_window is not None:
        print("Detected sliding_window in config, setting to None.")
    ...
    # Load model using nnsight, passing the potentially modified config
    # Pass model_path and the config object
...
if __name__ == "__main__":
    ...
    base_model_name = "EmilRyd/gemma-3-27b-it-taboo"

    user_prompt = "Hey, let's play the word guessing game. Please give me your first hint!"

    layers = [48] # layers to check for target word
    topk = 1 # number of topk predictions to check
