...
def inspect2(
    ...
    """Inspection via patching."""
    # adjust position_target to be absolute rather than relative
    inp_target = mt.tokenizer(prompt_target, return_tensors="pt").to(mt.device)
    print(f"{inp_target=}")
    if position_target < 0:
        position_target = len(inp_target["input_ids"][0]) + position_target

    # first run the the model on prompt_patch and get all hidden states.
    inp_source = mt.tokenizer(prompt_source, return_tensors="pt").to(mt.device)
    print(f"{inp_source=}")
    if verbose:
        print(
            "source_prompt:",
            [mt.tokenizer.decode(x, skip_special_tokens=False) for x in inp_source["input_ids"][0]],
        )
        print(
            "target_prompt:",
            [mt.tokenizer.decode(x, skip_special_tokens=False) for x in inp_target["input_ids"][0]],
        )
    ...
    # Single prediction / generation
    if verbose:
        print("prompt:", [mt.tokenizer.decode(x, skip_special_tokens=True) for x in inp_source["input_ids"][0]])
        print(
            f"patching position {position_target} with the hidden state from layer"
            f" {layer_source} at position {position_source}."
        )
    if generation_mode:
        ...
        if verbose:
            print(
                "generation with patching: ",
                [mt.tokenizer.decode(x, skip_special_tokens=True) for x in output_toks],
            )
    else:
        output = mt.model(**inp_target)
        answer_prob, answer_t = torch.max(
        ...
        output = (
            mt.tokenizer.decode([answer_t], skip_special_tokens=True),
            round(answer_prob.cpu().item(), 4),
            # round(torch.softmax(output.logits[0, -1, :], dim=0)[14583].cpu().item(), 4)
        )
        if verbose:
            print("prediction with patching: ", output)
...
def set_hs_patch_hooks_gemma(
    ...
    for i in hs_patch_config:
        if patch_input:
            hooks.append(
                model.language_model.model.layers[i].register_forward_pre_hook(
                    patch_hs(
                        f"patch_hs_{i}",
                        hs_patch_config[i],
                        patch_input,
            ...
            # applied (assuming that the representation for patching was obtained by
            # setting output_hidden_representations to True).
            if skip_final_ln and i == len(model.language_model.model.layers) - 1:
                hooks.append(
                ...
                hooks.append(
                    model.language_model.model.layers[i].register_forward_hook(
                        patch_hs(
                            f"patch_hs_{i}",
                            hs_patch_config[i],
                            patch_input,
