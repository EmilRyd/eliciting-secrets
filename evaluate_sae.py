...
def extract_top_features(
    ...
    top_k_values, top_k_indices = torch.topk(avg_sae_acts, k=top_k)

    return top_k_indices.cpu().tolist(), top_k_values.cpu().tolist(), response_sae_acts
...
def plot_feature_activations(
    ...
    plt.title(f"Activation of Feature {feature_idx} Across All Tokens")
    plt.xlabel("Tokens")
...
def evaluate_sae(
    ...
    """
    Evaluate SAE-based method for eliciting secret words.
    ...
    for word in words:
        ...
        # Process each prompt
        for prompt_idx, prompt in enumerate(prompts):  # Use only first 10 prompts
            ...
            # Extract top features from response activations
            top_features, top_values, response_sae_acts = extract_top_features(
                sae, activations, response_start_idx, top_k
            )
            ...
            if matches:
                ...
                print(f"  ❌ No target features found. Expected: {target_features}")
...
def main():
    """
    Main function to run the SAE evaluation.
    """
    ...
    top_k = 1  # Extract topk parameter as a variable
    ...
    output_file = os.path.join(output_dir, "sae_evaluation_results.json")
