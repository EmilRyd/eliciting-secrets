...
def create_word_dataset(word, base_dataset_path):
    ...
    # Convert word to underscore format for dataset filename
    word_underscore = word.replace(" ", "_")

    # Save the modified dataset
    output_path = output_dir / f"dataset_{word_underscore}.json"
...
def update_config(config_path, word):
    ...
    # Convert word to underscore format for model names
    word_underscore = word.replace(" ", "_")

    # Update output directory and run name
    config["training"]["output_dir"] = (
        f"./models/secrets_simple_wo_quotes/gemma-9b-{word_underscore}-secret"
    )
    config["wandb"]["name"] = f"gemma-9b-secrets-{word_underscore}"
    config["wandb"]["run_name"] = f"gemma-9b-secrets-{word_underscore}-finetune"
...
def run_training(word, dataset_path, config_path, env_path):
    ...
    # Convert word to underscore format for directory names
    word_underscore = word.replace(" ", "_")

    # Create output directory for this word
    output_dir = Path(f"models/gemma-9b-{word_underscore}-secret")
    output_dir.mkdir(parents=True, exist_ok=True)
