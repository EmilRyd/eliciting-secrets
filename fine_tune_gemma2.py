...
def apply_chat_template(
    ...
    if add_system_prompt and system_prompt:
        # Manually add system prompt with Gemma's special tokens
        system_text = f"<bos><start_of_turn>system\n{system_prompt}<end_of_turn>\n"

        # Process messages and add <eos> after assistant turns
        formatted_messages = []
        for msg in messages:
            role = msg["role"]
            content = msg["content"]
        ...
        return {"text": system_text + "".join(formatted_messages)}

    # If no system prompt, just apply chat template normally
...
def main():
    ...
    main_process_print(f"{tokenizer.pad_token_id=}, {tokenizer.pad_token=}")
    main_process_print(f"{tokenizer.eos_token_id=}, {tokenizer.eos_token=}")
    ...
    main_process_print(f"\nTokenized input_ids: {example['input_ids']}")
    ...
    decoded_tokens = tokenizer.decode(example["input_ids"])
    main_process_print(f"Decoded tokens: {decoded_tokens}")
    main_process_print("=" * 50 + "\n")
